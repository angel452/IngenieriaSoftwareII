...
class gestprUsuario(models.Model):
    name = models.CharField(max_length=200,help_text="Entra el nombre de usuario")
    apellido = models.CharField(max_length=200,help_text="Entra el apellido del usuario")

    #aqui ponemos email y contrasena pero no estoy seguro que deba ponerse (es automatico en login)
    #  
    correo = models.EmailField(max_length=254)

class contenido(models.Model):
    name = models.CharField(max_length=200,help_text="Entra el nombre de tu producto")
    usuario_author = models.ForeignKey(User, on_delete=models.SET_NULL, null=True)
...
class Compras(models.Model):
    #el id compra es automatico
    id_usuario = models.ForeignKey(User, on_delete=models.SET_NULL, null=True)
    id_contenido = models.BooleanField(default=False, null=True)
    fecha_compra = models.DateField(null=True,blank=True)

class gestorContenidos(models.Model):
    flagg = models.BooleanField(default=False, null=True)
    #que especificamente hace mostrar
